...
- **Voice-to-voice dubbing (Dub mode)** — drop an audio clip, transcribe it with Whisper, tidy the per-segment text, pick any voice, and **re-voice the whole clip** in that voice. Same-language or **cross-language** (translate first, then synthesize), audio-in → audio-out, with the original pauses preserved (natural timing, no time-stretch). Works on every engine.
- **Offline translation** — **M2M-100 418M** (~1 GB) and **M2M-100 1.2B** (~5 GB), both MIT, ~100 languages, plus **Argos Translate** (the open-source engine LibreTranslate wraps — 50 languages incl. Urdu, per-pair packs installed on demand). All run in-process and are selectable in the Translation panel. Powers cross-language dubbing and translating a transcript (with `.srt` / `.vtt` export) in Transcribe mode — nothing leaves your machine.
...
- **`max-text-chars` defaults to 5000.** Enforced app-wide in `SynthService`, so the cap is the same for every engine (not just VibeVoice) — over-limit requests return HTTP 400. It's an input guardrail, not a model context window: multi-line scripts are split per line before synthesis. Raise it with the `MAX_TEXT_CHARS` env var (or `backend/.env`); text well over 5K risks OOM on smaller GPUs.
